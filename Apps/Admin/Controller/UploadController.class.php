...
class UploadController extends Controller{
	...
	public function _initialize(){
		$this->userid=empty($_SESSION['uid'])? $_GET['userid'] : $_SESSION['uid'];
	...
	private function upFile($config,$fieldName){
		$conf=array(
			...
			'autoSub' => true,
			'subName'=>date('Ymd',time()) ,// 子目录命名的规则为 用户名/年月/
		...
		if($info){
			...
			if ($isimage) {
				...
				if ($this->water==1) {
					$image->text($this->waterText,'./Public/fonts/STXINGKA.TTF',18,'#9a9a9a',$this->waterPosition,array(-2,0))->save($fname); 
				}
				if ($this->water==2) {
