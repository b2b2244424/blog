...
app.controller('userCrtl', function ($scope, $http, $timeout, $location) {
    ...
    $scope.black = function(user){
        swal({
            title : '确定要拉黑'+user.username+'吗?',
            text : '确定要拉黑该用户?',
            type : 'warning',
            ...
            var uid = user.uid;
            $http.post('doBlack',{uid : uid}).success(function(data){
                if(data.code == 1){
                    swal("操作成功!", "已成功拉黑用户", "success");
